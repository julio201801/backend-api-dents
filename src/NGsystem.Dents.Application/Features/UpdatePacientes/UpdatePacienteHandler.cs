...
public class UpdatePacienteHandler
{
    ...
    public UpdatePacienteHandler(IValidator<PacienteUpdateRequestDto> validator, IPacienteRepository pacienteRepository, IPacienteService pacienteService)
    {
    ...
    public async Task<Result<PacienteUpdateResponseDTO>> Handle(PacienteUpdateRequestDto request)
    {
        ...
        // Buscar paciente
        var paciente = await _pacienteService.GetPacienteDtoAsync(request.Dni);
        if (paciente == null)
        {
